...
const LINKS = [
  ...
  },
  {
    ...
    icon: linksIconMirrorChyan,
  },
  {
    key: 'penguin-stats',
    href: 'https://penguin-stats.cn/',
    title: 'friendLinks.penguin-stats',
    icon: linksIconPenguinStats,
  },
  {
    key: 'yituliu',
    href: 'https://ark.yituliu.cn/',
    title: 'friendLinks.yituliu',
    icon: linksIconYituliu,
  },
  {
    key: 'arkntools',
    href: 'https://arkntools.app/',
    title: 'friendLinks.arkntools',
    icon: linksIconArkntools,
  },
  {
    key: 'ark-nights',
    href: 'https://ark-nights.com',
    title: 'friendLinks.ark-nights',
    icon: linksIconArkNights,
  },
  {
    key: 'alas',
    href: 'https://alas.azurlane.cloud/',
    title: 'friendLinks.alas',
    icon: mdiGitHub,
  },
]

export const HomeLinks = forwardRef<HTMLDivElement, HomeLinksProps>(
  ({ showLinks = false, onClose }, ref) => {
        ...
        if (showLinks) {
          ...
          if (isPanelCenteredRef.current) {
            ...
            panel.current.style.transform =
              'translate(-50%, -50%) translateY(10px) rotateX(5deg)' // 有点屎的实现，但能用
          } else {
            // 在右侧时向左横向淡出
    ...
    return (
      <div
        ...
        className={clsx(
          ...
          theme === 'dark' ? 'text-[#eee] bg-black' : 'text-gray-800 bg-white',
          styles.root,
        )}
