...
### Possible cause 2: incompatible CPU instruction set
 ...
 You can also try downloading `PaddleOCR` of [NoAVX](https://github.com/MaaAssistantArknights/MaaAssistantArknights/blob/master/3rdparty/ppocr_noavx.zip) version, unzip and replace the files with the same names. Since it may cause performance drop, please do not use it unless it is necessary.
...
Tips: please refer to the [List of the Supported Emulators](emulator-support.mdx) section to ensure that you are using your emulator correctly. If you are using Bluestack, please enable `Android Debug Bridge` in the settings.
...
1. Confirm that your emulator is supported in the [List of the Supported Emulators](emulator-support.mdx).
