...
- 可能性 2: CPU 指令集不支持。  
  ...
  您可以尝试下载 [NoAVX](https://github.com/MaaAssistantArknights/MaaAssistantArknights/3rdparty/ppocr_noavx.zip) 版本的 `PaddleOCR`, 解压后替换本软件中同名的文件。这是对于使用不支持 `AVX` 指令集的 CPU 的用户的性能降低的替代方案，如非必要，请不要使用。  
