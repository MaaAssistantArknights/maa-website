...
您可以尝试下载 [NoAVX](https://github.com/MaaAssistantArknights/MaaAssistantArknights/blob/master/3rdparty/ppocr_noavx.zip) 版本的 `PaddleOCR`, 解压后替换本软件中同名的文件。这是对于使用不支持 `AVX` 指令集的 CPU 的用户的性能降低的替代方案，如非必要，请不要使用。
...
**请注意，此项操作将会耗费您 10G 左右的磁盘空间，请务必在排查完毕其他可能性后使用。**

## 连接错误

提示 : 请参考 [模拟器支持](emulator-support.mdx) 确定您的模拟器是受支持的。如果是蓝叠模拟器，请注意在模拟器设置中打开 `Android 调试桥`
...
1. 请根据 [模拟器支持情况](emulator-support.mdx) 确定您的模拟器是受支持的
...
1. 请打开 [下载地址](https://github.com/MaaAssistantArknights/MaaAssistantArknights/blob/master/README.md#%E4%B8%8B%E8%BD%BD%E5%9C%B0%E5%9D%80) 中您所需要版本的链接（非镜像站）
2. 找到您所需要下载的文件链接
3. 右键该链接，选择 `复制链接地址`
4. 将链接地址粘贴到浏览器地址栏
